"""
Genome loading + one-file cache (DNA + Z-DNA labels + omics features).

Reading the per-chromosome DNA chunks and ~2000 per-feature pickles takes
minutes and happens on every job. This caches the parsed objects into a single
(compressed) joblib file, so later runs read one file instead of thousands.
...
subset and would not be reusable.

The feature column order MUST match the order used to train the saved weights.
The authors saved that order to "Список признаков.csv" (column 'feature').
Point env var OMIXAI_FEATURE_ORDER at that CSV to use it. Without it, the raw
os.listdir order is used (only correct if it equals the training order).

Disable the cache with env var OMIXAI_NO_GENOME_CACHE=1, or delete the cache file.
...
def _resolve_feature_order(features_dir: str) -> list[str]:
    ...
    Set env var OMIXAI_FEATURE_ORDER to the saved feature CSV (column 'feature')
    to use it. Falls back to raw os.listdir order if the env var is not set
    (only correct if listdir equals the training order).
    """
